...
#Load MIG pregnancy cohort (Primary)
mig_preg_cohort<-from_DP4_algorithm("migraine_algorithm_sensitivity/pregnancy_D3_sensitivity","MIG_Pregnancy_D3_S.rds")
...
if(!dir.exists(paste0(output_dir, "Drug utilisation/flowchart")))dir.create(paste0(output_dir, "Drug utilisation/flowchart"))

fwrite(flowchart,paste0(output_dir, "Drug utilisation/flowchart/Step_00_study_population_flowchart.csv"), row.names = F)
rm(flowchart)

preg_D3_during = copy(preg_D3)
###########################################################################
############ 12 month lookback period (Primary Analysis) ##############

####create covariates: Migraine type####
#Type 1
mig_type_1 = from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_T1_a_D3")
...
#Type 2
mig_type_2<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_T2_a_D3")
...
#Type 3
mig_type_3<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_T3_a_D3")
...
#Type 4
mig_type_4<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_T4_a_D3")
...
#Type 5
mig_type_5<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_T5_a_D3")
...
#Type 6
mig_type_6<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_T6_a_D3")
...
#Severity 1
mig_severity_1<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S1_a_D3")
...
#Severity 2
mig_severity_2<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S2_a_D3")
...
#Severity 3
mig_severity_3<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S3_a_D3")
...
#Severity 4 (we have S4 for during pregnancy only)
mig_severity_4<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S4_during_D3")
...
#Dx
mig_tmp = from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_Dx_a_D3")
if(length(mig_tmp)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
...
#DxRx
mig_tmp = from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_DxRx_a_D3")
if(length(mig_tmp)>0){
...
##############################################################################
############ 12 month lookback period (Primary Analysis) During ##############

preg_D3 = copy(preg_D3_during)
...
####create covariates: Migraine severity####
#Severity 1
mig_severity_1<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S1_during_D3")
if(length(mig_severity_1)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_severity_1<-mig_severity_1[,cols,with=F]
  preg_D3<-merge.data.table(preg_D3, mig_severity_1, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
...
#Severity 2
mig_severity_2<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S2_during_D3")
if(length(mig_severity_2)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_severity_2<-mig_severity_2[,cols,with=F]
  preg_D3<-merge.data.table(preg_D3, mig_severity_2, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
...
#Severity 3
mig_severity_3<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S3_during_D3")
if(length(mig_severity_3)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_severity_3<-mig_severity_3[,cols,with=F]
  preg_D3<-merge.data.table(preg_D3, mig_severity_3, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
...
#Dx
mig_tmp = from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_Dx_during_D3")
if(length(mig_tmp)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_tmp<-mig_tmp[,cols,with=F]
  preg_D3<-merge.data.table(preg_D3, mig_tmp, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
...
##############################################################################
############ 12 month lookback period (Primary Analysis) During2 ##############

preg_D3[, MIG_S1:=NULL][, MIG_S2:=NULL][, MIG_S3:=NULL][, MIG_S4:=NULL]

####create covariates: Migraine severity####
#Severity 1
mig_severity_1<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S1_during2_D3")
if(length(mig_severity_1)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_severity_1<-mig_severity_1[,cols,with=F]
  preg_D3<-merge.data.table(preg_D3, mig_severity_1, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
...
#Severity 2
mig_severity_2<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S2_during2_D3")
if(length(mig_severity_2)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_severity_2<-mig_severity_2[,cols,with=F]
  preg_D3<-merge.data.table(preg_D3, mig_severity_2, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
...
#Severity 3
mig_severity_3<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S3_during2_D3")
if(length(mig_severity_3)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_severity_3<-mig_severity_3[,cols,with=F]
  preg_D3<-merge.data.table(preg_D3, mig_severity_3, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
...
#Severity 4 (we have S4 for during pregnancy only)
mig_severity_4<-from_DP4_algorithm("migraine_algorithm_sensitivity/final_d3","MIG_S4_during2_D3")
if(length(mig_severity_4)>0){
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_severity_4<-mig_severity_4[,cols,with=F]
  preg_D3<-merge.data.table(preg_D3, mig_severity_4, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
...
#### Save preg cohort ####
#save pregnancy population to g_intermediate
saveRDS(preg_D3,paste0(g_intermediate,"pregnancy_D3/pregnancy_D3_DU_during2.rds"))

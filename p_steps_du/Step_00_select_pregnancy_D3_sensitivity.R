...
mig_preg_cohort<-from_DP4_algorithm("pregnancy_d3","MIG_Pregnancy_D3.rds")
...
fwrite(flowchart,paste0(output_dir, "Drug utilisation/flowchart/Step_00_study_population_flowchart_S.csv"), row.names = F)
rm(flowchart)

preg_D3_during = copy(preg_D3)

###########################################################################
############ 12 month lookback period (Sensitivity Analysis) ##############
...
saveRDS(preg_D3,paste0(g_intermediate,"pregnancy_D3/pregnancy_D3_DU_S.rds"))
...
saveRDS(min_obs, paste0(projectFolder, "/g_intermediate/pregnancy_d3/obs_period_hint_DU_S.rds"))

...
dir.create("g_export", showWarnings = FALSE)
...
# Loading data
preg_0 <- readRDS('g_intermediate/pregnancy_d3/pregnancy_d3.rds')  
triptans <- readRDS('g_intermediate/medicines_d3/TRIPTANS.rds') 
paracetamol <- readRDS('g_intermediate/medicines_d3/PARACETAMOL.rds') 
codeine <- readRDS('g_intermediate/medicines_d3/CODEINE_PARACETAMOL.rds') 
nsaid <- readRDS('g_intermediate/medicines_d3/NSAID.rds') 
antinausea <- readRDS('g_intermediate/medicines_d3/ANTI_NAUSEA.rds') 
prev_med <- readRDS('g_intermediate/medicines_d3/PREV_MIG_MED.rds') 
cgrp <- readRDS('g_intermediate/medicines_d3/CGRP_MED.rds') 
obesity <- readRDS('g_intermediate/diagnoses_d3/OW_fixed.rds') 
HT <- readRDS('g_intermediate/diagnoses_d3/BLHT_diagnoses.rds') 
diab <- readRDS('g_intermediate/diagnoses_d3/BLDM_diagnoses.rds') 
dep <- readRDS('g_intermediate/diagnoses_d3/BLDEP_diagnoses.rds') 

## Adding other covariates
# DxRx
#Type 2
mig_dxrx<-from_DP4_algorithm("migraine_algorithm/final_d3","MIG_DxRx_a_D3")
if(length(mig_dxrx)>0){
  setnames(mig_dxrx, "alternative", "include")
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_dxrx<-mig_dxrx[,cols,with=F]
  preg_0<-merge.data.table(preg_0, mig_dxrx, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
  setnames(preg_0, "include", "MIG_DxRx")
  preg_0[is.na(MIG_DxRx), MIG_DxRx:=0]
  rm(mig_dxrx)
}else{
  preg_0[,MIG_DxRx:=0]
}

# Should we have DxRx_S as well or it is needed in sensititivty analysis?
# DxRx_S
mig_dxrx_s<-from_DP4_algorithm("migraine_algorithm_sensitivity","MIG_DxRx_a_D3")
if(length(mig_dxrx_s)>0){
  setnames(mig_dxrx_s, "alternative", "include")
  cols<-c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date", "include")
  mig_dxrx_s<-mig_dxrx_s[,cols,with=F]
  preg_0<-merge.data.table(preg_0, mig_dxrx_s, by=c("person_id", "pregnancy_id", "pregnancy_start_date", "pregnancy_end_date"), all.x = T)
  setnames(preg_0, "include", "MIG_DxRx_S")
  preg_0[is.na(MIG_DxRx_S), MIG_DxRx_S:=0]
  rm(mig_dxrx_s)
}else{
  preg_0[,MIG_DxRx_S:=0]
}
...
elig_prim <- table(preg_0$MIG_DxRx_S)
print(elig_prim)    #Report on all migraineurs
elig_sens <- table(preg_0$MIG_DxRx)
print(elig_sens)  #Report on migraine at baseline only
DUS_all <- preg_0[preg_0$MIG_S2 == 1, ]  #Select population for primary analyses (all migraineurs)
...
DUS_all2$trip_T1_a <- ifelse((DUS_all2$gap_LMP >= 0 & DUS_all2$gap_LMP <= 97), 1, 0)
DUS_all2$trip_T2_a <- ifelse((DUS_all2$gap_LMP >= 98 & DUS_all2$gap_LMP <= 195), 1, 0)
...
write.csv(table1_prim, "g_export/Table1_primary.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table1_cons, "g_export/Table1_conservative.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table2_0912, "g_export/Table2_2009_2012.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table2_1316, "g_export/Table2_2013_2016.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table2_1720, "g_export/Table2_2017_2020.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table3_1524, "g_export/Table3_15_24.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table3_2534, "g_export/Table3_25_34.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table3_3549, "g_export/Table3_35_49.csv", row.names = FALSE)  ##Path to output folder
...
DUS_PCM$PCM_T1_a <- ifelse((DUS_PCM$gap_LMP >= 0 & DUS_PCM$gap_LMP <= 97), 1, 0)
DUS_PCM$PCM_T2_a <- ifelse((DUS_PCM$gap_LMP >= 98 & DUS_PCM$gap_LMP <= 195), 1, 0)
...
DUS_COD$COD_T1_a <- ifelse((DUS_COD$gap_LMP >= 0 & DUS_COD$gap_LMP <= 97), 1, 0)
DUS_COD$COD_T2_a <- ifelse((DUS_COD$gap_LMP >= 98 & DUS_COD$gap_LMP <= 195), 1, 0)
...
DUS_NSAID$NSAID_T1_a <- ifelse((DUS_NSAID$gap_LMP >= 0 & DUS_NSAID$gap_LMP <= 97), 1, 0)
DUS_NSAID$NSAID_T2_a <- ifelse((DUS_NSAID$gap_LMP >= 98 & DUS_NSAID$gap_LMP <= 195), 1, 0)
...
DUS_antinausea$antinausea_T1_a <- ifelse((DUS_antinausea$gap_LMP >= 0 & DUS_antinausea$gap_LMP <= 97), 1, 0)
DUS_antinausea$antinausea_T2_a <- ifelse((DUS_antinausea$gap_LMP >= 98 & DUS_antinausea$gap_LMP <= 195), 1, 0)
...
DUS_meto_prop$meto_prop_T1_a <- ifelse((DUS_meto_prop$gap_LMP >= 0 & DUS_meto_prop$gap_LMP <= 97), 1, 0)
DUS_meto_prop$meto_prop_T2_a <- ifelse((DUS_meto_prop$gap_LMP >= 98 & DUS_meto_prop$gap_LMP <= 195), 1, 0)
...
DUS_ami$ami_T1_a <- ifelse((DUS_ami$gap_LMP >= 0 & DUS_ami$gap_LMP <= 97), 1, 0)
DUS_ami$ami_T2_a <- ifelse((DUS_ami$gap_LMP >= 98 & DUS_ami$gap_LMP <= 195), 1, 0)
...
DUS_c_l_v$c_l_v_T1_a <- ifelse((DUS_c_l_v$gap_LMP >= 0 & DUS_c_l_v$gap_LMP <= 97), 1, 0)
DUS_c_l_v$c_l_v_T2_a <- ifelse((DUS_c_l_v$gap_LMP >= 98 & DUS_c_l_v$gap_LMP <= 195), 1, 0)
...
DUS_top_val$top_val_T1_a <- ifelse((DUS_top_val$gap_LMP >= 0 & DUS_top_val$gap_LMP <= 97), 1, 0)
DUS_top_val$top_val_T2_a <- ifelse((DUS_top_val$gap_LMP >= 98 & DUS_top_val$gap_LMP <= 195), 1, 0)
...
DUS_bot$bot_T1_a <- ifelse((DUS_bot$gap_LMP >= 0 & DUS_bot$gap_LMP <= 97), 1, 0)
DUS_bot$bot_T2_a <- ifelse((DUS_bot$gap_LMP >= 98 & DUS_bot$gap_LMP <= 195), 1, 0)
...
DUS_piz_clo$piz_clo_T1_a <- ifelse((DUS_piz_clo$gap_LMP >= 0 & DUS_piz_clo$gap_LMP <= 97), 1, 0)
DUS_piz_clo$piz_clo_T2_a <- ifelse((DUS_piz_clo$gap_LMP >= 98 & DUS_piz_clo$gap_LMP <= 195), 1, 0)
...
DUS_prev_med$prev_med_T1_a <- ifelse((DUS_prev_med$gap_LMP >= 0 & DUS_prev_med$gap_LMP <= 97), 1, 0)
DUS_prev_med$prev_med_T2_a <- ifelse((DUS_prev_med$gap_LMP >= 98 & DUS_prev_med$gap_LMP <= 195), 1, 0)
...
DUS_ere$ere_T1_a <- ifelse((DUS_ere$gap_LMP >= 0 & DUS_ere$gap_LMP <= 97), 1, 0)
DUS_ere$ere_T2_a <- ifelse((DUS_ere$gap_LMP >= 98 & DUS_ere$gap_LMP <= 195), 1, 0)
...
DUS_gal$gal_T1_a <- ifelse((DUS_gal$gap_LMP >= 0 & DUS_gal$gap_LMP <= 97), 1, 0)
DUS_gal$gal_T2_a <- ifelse((DUS_gal$gap_LMP >= 98 & DUS_gal$gap_LMP <= 195), 1, 0)
...
DUS_fre$fre_T1_a <- ifelse((DUS_fre$gap_LMP >= 0 & DUS_fre$gap_LMP <= 97), 1, 0)
DUS_fre$fre_T2_a <- ifelse((DUS_fre$gap_LMP >= 98 & DUS_fre$gap_LMP <= 195), 1, 0)
...
DUS_cgrp$cgrp_T1_a <- ifelse((DUS_cgrp$gap_LMP >= 0 & DUS_cgrp$gap_LMP <= 97), 1, 0)
DUS_cgrp$cgrp_T2_a <- ifelse((DUS_cgrp$gap_LMP >= 98 & DUS_cgrp$gap_LMP <= 195), 1, 0)
...
write.csv(table4_total, "g_export/Table4_total.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table4_continuer, "g_export/Table4_continuer.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table4_discontinuer, "g_export/Table4_discontinuer.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table4_nonuser, "g_export/Table4_nonuser.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table6, "g_export/Table6.csv", row.names = FALSE)  ##Path to output folder
...
DUS_obesity <- merge(DUS_all, obesity, by = "person_id", all.x = TRUE, allow.cartesian = TRUE) #many-to-many relationship
...
DUS_obesity1 = DUS_obesity %>%
  group_by(pregnancy_id) %>%
  summarize(obesity = max(obesity_a))
DUS_all <- merge(DUS_all, DUS_obesity1, by = c("pregnancy_id"))
DUS_all$obesity <- replace_na(DUS_all$obesity, 0)
rm("obesity", "DUS_obesity", "DUS_obesity1")

HT <- subset(HT, select = -c(pregnancy_id))
DUS_HT <- merge(DUS_all, HT, by = "person_id", all.x = TRUE, allow.cartesian = TRUE) #many-to-many relationship
...
DUS_diab <- merge(DUS_all, diab, by = "person_id", all.x = TRUE, allow.cartesian = TRUE) #many-to-many relationship
...
DUS_dep <- merge(DUS_all, dep, by = "person_id", all.x = TRUE, allow.cartesian = TRUE) #many-to-many relationship
...
#parity_1_n <- nrow(DUS_all[DUS_all$parity == "1+", ])
#parity_1_p <- round((parity_1_n / denom_all)*100, 1)
#parity_mis_n <- nrow(DUS_all[DUS_all$parity == "", ])
...
#non_parity_1_n <- nrow(nonusers[nonusers$parity == "1+", ])
#non_parity_1_p <- round((non_parity_1_n / denom_non)*100, 1)
#non_parity_mis_n <- nrow(nonusers[nonusers$parity == "", ])
...
#dis_parity_1_n <- nrow(discontinuers[discontinuers$parity == "1+", ])
#dis_parity_1_p <- round((dis_parity_1_n / denom_dis)*100, 1)
#dis_parity_mis_n <- nrow(discontinuers[discontinuers$parity == "", ])
...
#con_parity_1_n <- nrow(continuers[continuers$parity == "1+", ])
#con_parity_1_p <- round((con_parity_1_n / denom_con)*100, 1)
#con_parity_mis_n <- nrow(continuers[continuers$parity == "", ])
...
write.csv(table7, "g_export/Table7.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table8, "g_export/Table8.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table9, "g_export/Table9.csv", row.names = FALSE)  ##Path to output folder
...
write.csv(table10, "g_export/Table10.csv", row.names = FALSE)  ##Path to output folder

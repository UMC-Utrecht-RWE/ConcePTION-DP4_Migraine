...
if("MEDICAL_OBSERVATIONS" %in% unique(c(tables_diagnoses, tables_checkbox,tables_columns))){
  source(paste0(pre_dir, "Step_01_b_pre_diagnoses_clean_up_mo.R"))
  ...
  if(length(actual_tables$MEDICAL_OBSERVATIONS)>0){
    ...
    cols_needed<-unique(c(code_var, code_var_check, code_var_fix, voc_var, voc_var_check, voc_var_fix, date_var, date_var_check, date_var_fix, "mo_meaning", "person_id"))
    ...
    for (y in 1:length(actual_tables$MEDICAL_OBSERVATIONS)){
      ...
      #Load the table
      df<-fread(paste(path_dir, actual_tables$MEDICAL_OBSERVATIONS[y], sep=""), stringsAsFactors = FALSE, colClasses = "character", select = cols_needed)
      ...
      if(df[,.N]>0){
        ...
        df[mo_date>=obs_min & mo_date<=obs_max, keep:=1]
        ...
        if(df[,.N]>0){
          ...
          #du
          df[,prior_du:=ifelse(du_start_study_date>mo_date,1,0)]
          prior_ev_rec_du[[w]]<-df[prior_du==1,.N]
          df<-df[prior_du==0]
          df[,prior_du:=NULL]
          if(df[,.N]>0){
          df[,after_du:=ifelse(du_end_study_date<mo_date,1,0)]
          ...
          if(df[,.N]>0){
            ...
            if("MEDICAL_OBSERVATIONS" %in% tables_diagnoses){
              ...
              if(df[keep_diag==1,.N]>0){
                    table_name<-actual_tables$MEDICAL_OBSERVATIONS[y]
            ...
            if("MEDICAL_OBSERVATIONS" %in% tables_checkbox){
              ...
              if(df[keep_check==1,.N]>0){
                table_name<-actual_tables$MEDICAL_OBSERVATIONS[y]
                print(paste0("Filtering data for Hypertension checkbox:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                names_df<-names(df)
                if(!is.null(diag_checkbox_hyp_mo)){
                same_names<-intersect(names_df,names(diag_checkbox_hyp_mo))
                df<-merge.data.table(df,diag_checkbox_hyp_mo,by=same_names,all.x=T)
                if(df[!is.na(event_abbreviation),.N]>0){
                  setnames(df, date_var_check, "event_date")
                  setnames(df, "event_abbreviation", "variable_name")
                  
                  saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name")], paste0(projectFolder, "/g_intermediate/tmp/Checkbox_", unique(diag_checkbox_hyp_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Diabetes checkbox:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                if(!is.null(diag_checkbox_dm_mo)){
                  same_names<-intersect(names_df,names(diag_checkbox_dm_mo))
                  df<-merge.data.table(df,diag_checkbox_dm_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_check, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name")], paste0(projectFolder, "/g_intermediate/tmp/Checkbox_", unique(diag_checkbox_dm_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Depression checkbox:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                if(!is.null(diag_checkbox_dep_mo)){
                  same_names<-intersect(names_df,names(diag_checkbox_dep_mo))
                  df<-merge.data.table(df,diag_checkbox_dep_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_check, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name")], paste0(projectFolder, "/g_intermediate/tmp/Checkbox_", unique(diag_checkbox_dep_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Parity checkbox:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                if(!is.null(diag_checkbox_parity_mo)){
                  same_names<-intersect(names_df,names(diag_checkbox_parity_mo))
                  df<-merge.data.table(df,diag_checkbox_parity_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_check, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name")], paste0(projectFolder, "/g_intermediate/tmp/Checkbox_", unique(diag_checkbox_parity_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Obesity checkbox:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                if(!is.null(diag_checkbox_obesity_mo)){
                  same_names<-intersect(names_df,names(diag_checkbox_obesity_mo))
                  df<-merge.data.table(df,diag_checkbox_obesity_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_check, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name")], paste0(projectFolder, "/g_intermediate/tmp/Checkbox_", unique(diag_checkbox_obesity_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
            ...
            if("MEDICAL_OBSERVATIONS" %in% tables_columns){
              ...
              if(df[keep_fix==1,.N]>0){
                table_name<-actual_tables$MEDICAL_OBSERVATIONS[y]
                print(paste0("Filtering data for Hypertension fixed columns:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                names_df<-names(df)
                if(!is.null(diag_fixed_hyp_mo)){
                  same_names<-intersect(names_df,names(diag_fixed_hyp_mo))
                  df<-merge.data.table(df,diag_fixed_hyp_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_fix, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    col_keep<-unique(diag_fixed_hyp_mo[,keep])
                    setnames(df, col_keep, "event_code")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name", "event_code")], paste0(projectFolder, "/g_intermediate/tmp/Fixed_", unique(diag_fixed_hyp_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Diabetes fixed columns:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                if(!is.null(diag_fixed_dm_mo)){
                  same_names<-intersect(names_df,names(diag_fixed_dm_mo))
                  df<-merge.data.table(df,diag_fixed_dm_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_fix, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    col_keep<-unique(diag_fixed_dm_mo[,keep])
                    setnames(df, col_keep, "event_code")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name", "event_code")], paste0(projectFolder, "/g_intermediate/tmp/Fixed_", unique(diag_fixed_dm_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Depression fixed columns:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                if(!is.null(diag_fixed_dep_mo)){
                  same_names<-intersect(names_df,names(diag_fixed_dep_mo))
                  df<-merge.data.table(df,diag_fixed_dep_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_fix, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    col_keep<-unique(diag_fixed_dep_mo[,keep])
                    setnames(df, col_keep, "event_code")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name", "event_code")], paste0(projectFolder, "/g_intermediate/tmp/Fixed_", unique(diag_fixed_dep_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Parity fixed columns:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                if(!is.null(diag_fixed_parity_mo)){
                  same_names<-intersect(names_df,names(diag_fixed_parity_mo))
                  df<-merge.data.table(df,diag_fixed_parity_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_fix, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    col_keep<-unique(diag_fixed_parity_mo[,keep])
                    setnames(df, col_keep, "event_code")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name", "event_code")], paste0(projectFolder, "/g_intermediate/tmp/Fixed_", unique(diag_fixed_parity_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Obesity fixed columns:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                names_df<-names(df)
                if(!is.null(diag_fixed_bmi_mo)){
                  same_names<-intersect(names_df,names(diag_fixed_bmi_mo))
                  df<-merge.data.table(df,diag_fixed_bmi_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_fix, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    col_keep<-unique(diag_fixed_bmi_mo[,keep])
                    setnames(df, col_keep, "event_code")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name", "event_code")], paste0(projectFolder, "/g_intermediate/tmp/Fixed_", unique(diag_fixed_bmi_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Height fixed columns:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                names_df<-names(df)
                if(!is.null(diag_fixed_h_mo)){
                  same_names<-intersect(names_df,names(diag_fixed_h_mo))
                  df<-merge.data.table(df,diag_fixed_h_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_fix, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    col_keep<-unique(diag_fixed_h_mo[,keep])
                    setnames(df, col_keep, "event_code")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name", "event_code")], paste0(projectFolder, "/g_intermediate/tmp/Fixed_", unique(diag_fixed_h_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
                ...
                print(paste0("Filtering data for Weight fixed columns:", actual_tables$MEDICAL_OBSERVATIONS[y]))
                names_df<-names(df)
                if(!is.null(diag_fixed_w_mo)){
                  same_names<-intersect(names_df,names(diag_fixed_w_mo))
                  df<-merge.data.table(df,diag_fixed_w_mo,by=same_names,all.x=T)
                  if(df[!is.na(event_abbreviation),.N]>0){
                    setnames(df, date_var_fix, "event_date")
                    setnames(df, "event_abbreviation", "variable_name")
                    col_keep<-unique(diag_fixed_w_mo[,keep])
                    setnames(df, col_keep, "event_code")
                    
                    saveRDS(df[!is.na(variable_name), c("person_id", "event_date", "variable_name", "event_code")], paste0(projectFolder, "/g_intermediate/tmp/Fixed_", unique(diag_fixed_w_mo[,event_abbreviation]), "_", actual_tables$MEDICAL_OBSERVATIONS[y], ".rds"))
    ...
    flowchart_events_du<-data.table(Indicator=c("Number of original rows",
                                    ...
                                    MEDICAL_OBSERVATIONS=c(original_rows_du,
                                                           empty_ev_date_du,
                                                          empty_meaning_du,
                                                          any_study_no,
                                                          outside_obs,
    ...
    fwrite(flowchart_events_du, paste0(projectFolder, "/g_output/Drug utilisation/flowchart/mo_table_flowchart.csv"), row.names = F)
  ...
  flowchart_events_du<-data.table(Indicator=c("Number of original rows",
                                              ...
                                              "Number of records included, before data filtering for checkbox information",
                                              "Number of records included, before data filtering for column information"),
                                  MEDICAL_OBSERVATIONS=rep("N/A",16))

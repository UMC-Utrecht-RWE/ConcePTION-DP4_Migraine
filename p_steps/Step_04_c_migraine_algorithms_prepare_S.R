...
pregnancy_d3_mig_bc = copy(pregnancy_d3_mig)

pregnancy_d3_mig = pregnancy_d3_mig[, .(person_id, pregnancy_id, pregnancy_start_date, pregnancy_end_date, year, birth_date, death_date,
                                        age, maternal_age, year_group,
                                        op_start_date_mig, op_end_date_mig)]
...
mig_files<-mig_files[!mig_files %in% c("Migraine_medicines_S.rds", "Migraine_medicines.rds")]
mig_files<-mig_files[!grepl("MIG_[D|R]x.*", mig_files, perl = TRUE)]
names_events<-list()
...
files_mg_list<-mig_files[str_detect(mig_files,"MG.rds")]

for(mg_i in seq_along(mig_files)){
  files_mg<-mig_files[mg_i]
  names_mg<-gsub("(\\_S\\.rds)|(\\.rds)", "", files_mg)
...
if(mig_dt[,.N]>0){
  ...
  if(mig_dt[,.N]>0){
    ...
    saveRDS(mig_dt[,names_mig_dt, with=F],paste0(projectFolder, "/g_intermediate/migraine_algorithm_sensitivity/",names_mg,"_S.rds"))
...
if(length(mig_med_fl)>0){
  ...
  if(mig_med[,.N]>0){
    ...
    if(mig_med[,.N]>0){
      ...
      saveRDS(mig_med[,names_mig_med, with=F],paste0(projectFolder, "/g_intermediate/migraine_algorithm_sensitivity/Migraine_medicines_S.rds"))
      rm(mig_med)
    }
  }
...
saveRDS(pregnancy_d3_mig,paste0(projectFolder,"/g_intermediate/pregnancy_d3/MIG_Pregnancy_D3_S.rds"))
####Migraine DIAGNOSES####
original<-list()
before<-list()
after<-list()
...
if(!dir.exists(paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/final_d3")))
dir.create(paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/final_d3"), recursive = TRUE)
...
if(!dir.exists(paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/pregnancy_D3_sensitivity/final_d3")))
dir.create(paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/pregnancy_D3_sensitivity/final_d3"), recursive = TRUE)

####Migraine DIAGNOSES sensitivity####
mig_files<-list.files(paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/"), "MG.*_S.rds")
names_events<-gsub("_S.rds","", mig_files,fixed = TRUE)
w<-1
for(mig_fl in 1:length(mig_files)){
  
  
  
  mig_dt<-readRDS(paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/", mig_files[mig_fl]))
  #merge with the pregnancy d3
  mig_dt<-merge.data.table(pregnancy_d3_mig, mig_dt, by="person_id", all.x=T, allow.cartesian = T)
  mig_dt<-mig_dt[!is.na(event_date)]
  if(mig_dt[,.N]>0){
...
if(length(mig_files_checkbox)>0){
  ...
  age_band<-Vectorize(age_band_creation)(preg_d3_checkbox$age)
  ...
  for(mig_fl in 1:length(mig_files_checkbox)){
    ...
    if(mig_dt[,.N]>0){
      ...
      if(mig_dt[,.N]>0){
        ...
        saveRDS(mig_dt, paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/final_d3/", "Migraine_checkbox_pregnancy_D3.rds"))
        cols<-c("person_id","pregnancy_id","pregnancy_start_date","pregnancy_end_date","event_date")
        mig_dt<-mig_dt[,cols,with=F]
...
# rm(mig_files)
# fwrite(sum_mig,paste0(projectFolder,"/g_output/Migraine algorithm/Step_04_summary_included_records_migraine_S.csv"),row.names = F)
# rm(sum_mig)
...
for(med_fl in 1:length(obs_period_med[,.N])){
  if(mig_med[,.N]>0){
    ...
    if(mig_med[,.N]>0){
      ...
      saveRDS(mig_med[,cols_to_exp,with=F], paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/final_d3/", obs_period_med[med_fl,StudyVar],"_pregnancy_D3.rds"))
      saveRDS(mig_med[injection==1,cols_to_exp,with=F], paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/final_d3/", "Migraine_medicines_injections_pregnancy_D3.rds"))
      rm(cols_to_exp)
...
if(mig_med_other[,.N]>0){
  ...
  if(mig_med_other[,.N]>0){
    ...
    cols_to_exp<-c("person_id","pregnancy_id","pregnancy_start_date","pregnancy_end_date","year","birth_date","death_date","op_end_date_mig","op_start_date_mig","age","maternal_age","year_group","medicine_date","atc_code","condition","medicinal_product_group")
    saveRDS(mig_med_other[,cols_to_exp,with=F], paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/final_d3/Migraine_medicines_profilactic_pregnancy_D3.rds"))
    rm(cols_to_exp)
...
fwrite(removed_rec,paste0(projectFolder,"/g_output/Migraine algorithm/Step_04_excluded_records_migraine_S.csv"),row.names = F)
rm(removed_rec)
...
fwrite(obs_period, paste0(projectFolder,"/g_output/Migraine algorithm/Step_04_observation_periods_migraine_S.csv"),row.names = F)
...
fwrite(pregnancy_d3_mig,paste0(projectFolder,"/g_intermediate/migraine_algorithm_sensitivity/final_d3/pregnancy_d3_migraine_algorithm.csv"),row.names = F)

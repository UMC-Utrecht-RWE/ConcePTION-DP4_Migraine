...
#export MIG_Dx_first
MIG_Dx_first_dt<-data.table(algorithm="MIG_Dx_first", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=97][include==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id),.N])
...
#by maternal age_band
pregnancy_d3_mig[,age_band:=as.character(age_band)]
records<-pregnancy_d3_mig[GA>=97][include==1 & !duplicated(pregnancy_id), .N, by=.(age_band)]
names(records)<-c("age_band","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id), by="age_band",.N]
names(total)<-c("age_band","no_pregnancies")
records<-merge.data.table(records,total,by="age_band",all=T)
...
#by year to year_group
records<-pregnancy_d3_mig[GA>=97][include==1 & !duplicated(pregnancy_id),by="year_group", .N]
names(records)<-c("year_group","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id),by="year_group",.N]
names(total)<-c("year_group","no_pregnancies")
records<-merge.data.table(records,total,by="year_group",all=T)
...
#by year group and maternal age_band
records<-pregnancy_d3_mig[GA>=97][include==1 & !duplicated(pregnancy_id),by=c("year_group","age_band"), .N]
names(records)<-c("year_group","age_band", "no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id),by=c("year_group","age_band"),.N]
names(total)<-c("year_group","age_band","no_pregnancies")
records<-merge.data.table(records,total,by=c("year_group","age_band"), all=T)
...
#export MIG_Rx_first
MIG_Rx_first_dt<-data.table(algorithm="MIG_Rx_first", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=97][include==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id),.N])
...
#export MIG_Dx_a
MIG_DxRx_first_dt<-data.table(algorithm="MIG_DxRx_first", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=97][alternative==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id),.N])
...
#by maternal age_band
pregnancy_d3_mig[,age_band:=as.character(age_band)]
records<-pregnancy_d3_mig[GA>=97][alternative==1 & !duplicated(pregnancy_id), .N, by=.(age_band)]
names(records)<-c("age_band","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id), by="age_band",.N]
names(total)<-c("age_band","no_pregnancies")
records<-merge.data.table(records,total,by="age_band",all=T)
...
#by year to year_group
records<-pregnancy_d3_mig[GA>=97][alternative==1 & !duplicated(pregnancy_id),by="year_group", .N]
names(records)<-c("year_group","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id),by="year_group",.N]
names(total)<-c("year_group","no_pregnancies")
records<-merge.data.table(records,total,by="year_group",all=T)
...
#by year group and maternal age_band
records<-pregnancy_d3_mig[GA>=97][alternative==1 & !duplicated(pregnancy_id),by=c("year_group","age_band"), .N]
names(records)<-c("year_group","age_band", "no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=97][!duplicated(pregnancy_id),by=c("year_group","age_band"),.N]
names(total)<-c("year_group","age_band","no_pregnancies")
records<-merge.data.table(records,total,by=c("year_group","age_band"), all=T)
...
MIG_Dx_second_dt<-data.table(algorithm="MIG_Dx_second", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=195][include==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id),.N])
...
#by maternal age_band
pregnancy_d3_mig[,age_band:=as.character(age_band)]
records<-pregnancy_d3_mig[GA>=195][include==1 & !duplicated(pregnancy_id), .N, by=.(age_band)]
names(records)<-c("age_band","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id), by="age_band",.N]
names(total)<-c("age_band","no_pregnancies")
records<-merge.data.table(records,total,by="age_band",all=T)
...
#by year to year_group
records<-pregnancy_d3_mig[GA>=195][include==1 & !duplicated(pregnancy_id),by="year_group", .N]
names(records)<-c("year_group","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id),by="year_group",.N]
names(total)<-c("year_group","no_pregnancies")
records<-merge.data.table(records,total,by="year_group",all=T)
...
#by year group and maternal age_band
records<-pregnancy_d3_mig[GA>=195][include==1 & !duplicated(pregnancy_id),by=c("year_group","age_band"), .N]
names(records)<-c("year_group","age_band", "no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id),by=c("year_group","age_band"),.N]
names(total)<-c("year_group","age_band","no_pregnancies")
records<-merge.data.table(records,total,by=c("year_group","age_band"), all=T)
...
MIG_Rx_second_dt<-data.table(algorithm="MIG_Rx_second", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=195][include==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id),.N])
...
if(length(alt_col)>0){pregnancy_d3_mig[pregnancy_d3_mig[,Reduce("|" , lapply(.SD,`>=`, 1)),.SDcols=alt_col] & GA>=195,alternative:=1]}else{pregnancy_d3_mig[,alternative:=NA]}

saveRDS(pregnancy_d3_mig,paste0(projectFolder,"/g_intermediate/migraine_algorithm/final_d3/MIG_DxRx_second_D3.rds"))

#export MIG_Dx_a
MIG_DxRx_second_dt<-data.table(algorithm="MIG_DxRx_second", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=195][alternative==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id),.N])
...
#by maternal age_band
pregnancy_d3_mig[,age_band:=as.character(age_band)]
records<-pregnancy_d3_mig[GA>=195][alternative==1 & !duplicated(pregnancy_id), .N, by=.(age_band)]
names(records)<-c("age_band","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id), by="age_band",.N]
names(total)<-c("age_band","no_pregnancies")
records<-merge.data.table(records,total,by="age_band",all=T)
...
#by year to year_group
records<-pregnancy_d3_mig[GA>=195][alternative==1 & !duplicated(pregnancy_id),by="year_group", .N]
names(records)<-c("year_group","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id),by="year_group",.N]
names(total)<-c("year_group","no_pregnancies")
records<-merge.data.table(records,total,by="year_group",all=T)
...
#by year group and maternal age_band
records<-pregnancy_d3_mig[GA>=195][alternative==1 & !duplicated(pregnancy_id),by=c("year_group","age_band"), .N]
names(records)<-c("year_group","age_band", "no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=195][!duplicated(pregnancy_id),by=c("year_group","age_band"),.N]
names(total)<-c("year_group","age_band","no_pregnancies")
records<-merge.data.table(records,total,by=c("year_group","age_band"), all=T)
...
#export MIG_Dx_third
MIG_Dx_third_dt<-data.table(algorithm="MIG_Dx_third", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=196][include==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id),.N])
...
#by maternal age_band
pregnancy_d3_mig[,age_band:=as.character(age_band)]
records<-pregnancy_d3_mig[GA>=196][include==1 & !duplicated(pregnancy_id), .N, by=.(age_band)]
names(records)<-c("age_band","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id), by="age_band",.N]
names(total)<-c("age_band","no_pregnancies")
records<-merge.data.table(records,total,by="age_band",all=T)
...
#by year to year_group
records<-pregnancy_d3_mig[GA>=196][include==1 & !duplicated(pregnancy_id),by="year_group", .N]
names(records)<-c("year_group","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id),by="year_group",.N]
names(total)<-c("year_group","no_pregnancies")
records<-merge.data.table(records,total,by="year_group",all=T)
...
#by year group and maternal age_band
records<-pregnancy_d3_mig[GA>=196][include==1 & !duplicated(pregnancy_id),by=c("year_group","age_band"), .N]
names(records)<-c("year_group","age_band", "no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id),by=c("year_group","age_band"),.N]
names(total)<-c("year_group","age_band","no_pregnancies")
records<-merge.data.table(records,total,by=c("year_group","age_band"), all=T)
...
#export MIG_Rx_third
MIG_Rx_third_dt<-data.table(algorithm="MIG_Rx_third", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=196][include==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id),.N])
...
#export MIG_Dx_a
MIG_DxRx_third_dt<-data.table(algorithm="MIG_DxRx_third", no_diagnosed_pregnancies=pregnancy_d3_mig[GA>=196][alternative==1 & !duplicated(pregnancy_id),.N], no_pregnancies=pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id),.N])
...
#by maternal age_band
pregnancy_d3_mig[,age_band:=as.character(age_band)]
records<-pregnancy_d3_mig[GA>=196][alternative==1 & !duplicated(pregnancy_id), .N, by=.(age_band)]
names(records)<-c("age_band","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id), by="age_band",.N]
names(total)<-c("age_band","no_pregnancies")
records<-merge.data.table(records,total,by="age_band",all=T)
...
#by year to year_group
records<-pregnancy_d3_mig[GA>=196][alternative==1 & !duplicated(pregnancy_id),by="year_group", .N]
names(records)<-c("year_group","no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id),by="year_group",.N]
names(total)<-c("year_group","no_pregnancies")
records<-merge.data.table(records,total,by="year_group",all=T)
...
#by year group and maternal age_band
records<-pregnancy_d3_mig[GA>=196][alternative==1 & !duplicated(pregnancy_id),by=c("year_group","age_band"), .N]
names(records)<-c("year_group","age_band", "no_diagnosed_pregnancies")
total<-pregnancy_d3_mig[GA>=196][!duplicated(pregnancy_id),by=c("year_group","age_band"),.N]
names(total)<-c("year_group","age_band","no_pregnancies")
records<-merge.data.table(records,total,by=c("year_group","age_band"), all=T)

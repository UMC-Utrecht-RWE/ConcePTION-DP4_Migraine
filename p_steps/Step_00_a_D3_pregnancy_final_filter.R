...
D3_filtered <- rbind(pregnancy_D3[(meaning_of_principal_record == "birth_registry_mother" & imputed_end_of_pregnancy == 0)],
                     pregnancy_D3[meaning_of_principal_record != "birth_registry_mother"][type_of_pregnancy_end != "LB" & type_of_pregnancy_end != "SB"][imputed_start_of_pregnancy == 0 | imputed_end_of_pregnancy ==0])
rm(pregnancy_D3)
...
if(data_source_name=="USWAN"){
D3_filtered <- pregnancy_D3[imputed_end_of_pregnancy == 0][PROMPT == "yes" | EUROCAT == "yes"]
...
D3_filtered <- rbind(pregnancy_D3[PROMPT == "yes" & imputed_end_of_pregnancy == 0],
                     pregnancy_D3[meaning_of_principal_record %in% list_of_meanings & imputed_end_of_pregnancy == 0])
...
if(data_source_name=="NIHW"){
list_of_meanings_THL = c("birth_registry",
                         "induced_termination_registry",
                         "birth_registry_child")

D3_filtered <- pregnancy_D3[meaning_of_principal_record %in% list_of_meanings]

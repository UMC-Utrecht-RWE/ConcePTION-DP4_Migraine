...
if(sum(!is.null(sum_gdm_dm_preg),!is.null(sum_gdm_unk_gd),!is.null(sum_gdm_pre_gd),!is.null(sum_gdm_gd),!is.null(sum_gdm_dm),!is.null(sum_gdm))){
  ...
  fwrite(sum,paste0(output_dir,"PE and GDM algorithm/summary_gdm_icluded_record.csv"),row.names = F)
...
if(sum(!is.null(dup_dt_hellp),!is.null(dup_dt_ecl),!is.null(dup_dt_pe),!is.null(dup_gdm_diag))){
  ...
  fwrite(dup,paste0(output_dir,"PE and GDM algorithm/gdm_pe_duplicated_diagnoses_removed.csv"),row.names = F)
  rm(dup)
}
...
if(sum(!is.null(sum_hellp),!is.null(sum_ecl),!is.null(sum_dt_pe),!is.null(sum_pe))){
  ...
  fwrite(sum,paste0(output_dir,"PE and GDM algorithm/summary_pe_icluded_record.csv"),row.names = F)
...
#OTHER_MG
other_mg_files<-list.files(tmp, "_OTHER_MG_")
if(length(other_mg_files)>0){
  ...
  if(dup>0){
    dup_dt_other_mg<-data.table(StudyVar="Migraine_diagnoses/OTHER_MG", removed_rec=dup)
  }else{
    dup_dt_other_mg<-data.table(StudyVar="Migraine_diagnoses/OTHER_MG", removed_rec=0)  
  }
  rm(dup)
  ...
  #fwrite(sum,paste0(output_dir,"OTHER_MG and GDM algorithm/summary_gdm_diagnoses_pre_gd.csv"))
  ...
  #save data in g_intermediate/populations
  saveRDS(other_mg_dt, paste0(g_intermediate,"migraine_algorithm/OTHER_MG.rds"))
  rm(other_mg_dt)
  #remove files
  #remove all gdm files from tmp
  ...
  dup_dt_other_mg<-data.table(StudyVar="Migraine_diagnoses/OTHER_MG", removed_rec="N/A")
  sum_dt_other_mg<-NULL
}
rm(other_mg_files)

#UNSP_MG
unsp_mg_files<-list.files(tmp, "_UNSP_MG_")
if(length(unsp_mg_files)>0){
  ...
  if(dup>0){
    dup_dt_unsp_mg<-data.table(StudyVar="Migraine_diagnoses/UNSP_MG", removed_rec=dup)
  }else{
    dup_dt_unsp_mg<-data.table(StudyVar="Migraine_diagnoses/UNSP_MG", removed_rec=0)  
  }
  rm(dup)
  ...
  #save data in g_intermediate/populations
  saveRDS(unsp_mg_dt, paste0(g_intermediate,"migraine_algorithm/UNSP_MG.rds"))
  rm(unsp_mg_dt)
  #remove files
  #remove all gdm files from tmp
  ...
  dup_dt_unsp_mg<-data.table(StudyVar="Migraine_diagnoses/UNSP_MG", removed_rec="N/A")
  sum_dt_unsp_mg<-NULL
}
rm(unsp_mg_files)

#COMP_MG
comp_mg_files<-list.files(tmp, "_COMP_MG_")
if(length(comp_mg_files)>0){
  ...
  if(dup>0){
    dup_dt_comp_mg<-data.table(StudyVar="Migraine_diagnoses/COMP_MG", removed_rec=dup)
  }else{
    dup_dt_comp_mg<-data.table(StudyVar="Migraine_diagnoses/COMP_MG", removed_rec=0)  
  }
  rm(dup)
  ...
  #save data in g_intermediate/populations
  saveRDS(comp_mg_dt, paste0(g_intermediate,"migraine_algorithm/COMP_MG.rds"))
  rm(comp_mg_dt)
  #remove files
  #remove all gdm files from tmp
  ...
  dup_dt_comp_mg<-data.table(StudyVar="Migraine_diagnoses/COMP_MG", removed_rec="N/A")
  sum_dt_comp_mg<-NULL
}
rm(comp_mg_files)
...
if(sum(!is.null(dup_dt_mg_no_aura),!is.null(dup_dt_mg_aura),!is.null(dup_dt_mg_status),!is.null(dup_dt_other_mg),!is.null(dup_dt_unsp_mg),!is.null(dup_dt_comp_mg),!is.null(dup_dt_mg))){
  ...
  fwrite(dup,paste0(output_dir,"Migraine algorithm/mig_duplicated_diagnoses_removed.csv"),row.names = F)
  rm(dup)
}
...
if(sum(!is.null(sum_dt_mg_no_aura),!is.null(sum_dt_mg_aura),!is.null(sum_dt_mg_status),!is.null(sum_dt_other_mg),!is.null(sum_dt_unsp_mg),!is.null(sum_dt_comp_mg),!is.null(sum_dt_mg))){
  ...
  fwrite(sum,paste0(output_dir,"Migraine algorithm/summary_mig_icluded_record.csv"),row.names = F)

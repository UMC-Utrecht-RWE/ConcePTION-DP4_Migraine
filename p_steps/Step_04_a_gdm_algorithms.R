...
for(gdm_fl in 1:length(gdm_files)){
  ...
  if(gdm_dt[,.N]>0){
    ...
    gdm_dt[,lookback:=as.numeric(obs_period_diag[StudyVar==names_events[gdm_fl],lookback])]
    gdm_dt[,start_preg:=as.IDate(get(obs_period_diag[StudyVar==names_events[gdm_fl],start_date])- lookback)]
    gdm_dt[,start_preg:=start_preg + as.numeric(obs_period_diag[StudyVar==names_events[gdm_fl],add_start])]
    ...
    if(gdm_dt[,.N]>0){
      gdm_dt[,after:=as.numeric(obs_period_diag[StudyVar==names_events[gdm_fl],after])]
      gdm_dt[,end_preg:=as.IDate(get(obs_period_diag[StudyVar==names_events[gdm_fl],end_date]) + after)]
      #exclude all pregnancies that are outside observation period of interest
...
if(DAP_name=="CHUT"){
  obs_period_med<-data.table(StudyVar=c("GD_med","PRE_GD_med"),
                             lookback=c(0,3*30.4375-14),
                             start_date="pregnancy_start_date",
                             add_start=c(97,0),
                             end_date=c("pregnancy_end_date", "pregnancy_start_date"),
                             after=c(0,97))
}else{
  obs_period_med<-data.table(StudyVar=c("GD_med","PRE_GD_med"),
...
records<-pregnancy_d3_gdm_pe[include==1 & is.na(exclude) & !duplicated(pregnancy_id),by="year_group", .N]
...
GDM_1_c<-GDM_1_c[order(year_group)]
rm(records,total)
...
#Change year to year_group
records<-pregnancy_d3_gdm_pe[alternative==1 & is.na(exclude) & !duplicated(pregnancy_id),by="year_group", .N]
names(records)<-c("year_group","no_diagnosed_pregnancies")
total<-pregnancy_d3_gdm_pe[!duplicated(pregnancy_id),by="year_group",.N]
names(total)<-c("year_group","no_pregnancies")
records<-merge.data.table(records,total,by="year_group",all=T)
...
GDM_2_c<-GDM_2_c[order(year_group)]
rm(records,total)
...
GDM_3_c<-GDM_3_c[order(year_group)]
rm(records,total)
...
GDM_4_c<-GDM_4_c[order(year_group)]
rm(records,total)
...
GDM_5_c<-GDM_5_c[order(year_group)]
rm(records,total)
...
GDM_8_c<-GDM_8_c[order(year_group)]
rm(records,total)
